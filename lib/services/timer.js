...
export class Timer {
  ...
  start({ run_callback=true }={})  {
    if(!this.started) {
      this.started = true;
      if(run_callback) this._runCallback("start");
  ...
  pause({ run_callback=true }={})  {
    if(this.started && this.paused_at == null) {
      this.paused_at = Timer.time_now_in_ms;
      if(run_callback) this._runCallback("pause");
   }
  }

  resume({ run_callback=true }={}) {
    if(this.paused_at != null) {
      ...
      this.paused_at = null;
      if(run_callback) this._runCallback("resume");
    }
  }

  finish({ run_callback=true }={}) {
    this.finished = true;
    if(this.started && run_callback) this._runCallback("finish");
  }

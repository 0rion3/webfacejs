...
export const ComponentDom = (ComponentDom) => class extends ComponentDom {
  ...
  _readAttrFromNode(attr_name, { run_callback=false }={}) {
    ...
    if(this.non_dom_attribute_names.includes(attr_name))
      throw AttributeIsNonDomError(`The attribute you're attempting to read from DOM (${attr_name}) is marked as non-DOM attribute.`)
    ...
    if(attr_el != null) {
      ...
      if(v) this.set(attr_name, this._castAttrFromDom(attr_name, v), { run_callback: run_callback });
    }
  }
  ...
  _writeAttrToNode(attr_name) {
    ...
    if(this.non_dom_attribute_names.includes(attr_name))
      throw AttributeIsNonDomError(`The attribute you're attempting to write to DOM (${attr_name}) is marked as non-DOM attribute.`)

    if(this.dom_element == null) return;
    if(this.get(attr_name) === undefined) return;

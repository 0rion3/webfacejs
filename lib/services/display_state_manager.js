...
export class DisplayStateManager {
  ...
  applyAction(action, entities, { animation_speed=null }={}) {
    ...
    entities.forEach(entity_name => {
      ...
      if(typeof entity === "string") {
        console.log(`${action}ing part ${entity}`);
  ...
  _extractChildComponentRoles() {
    var roles = [];
    this.display_states.forEach(declaration => {
      Object.keys(declaration[0]).forEach((attr_name) => {
        if(attr_name.includes("."))
          roles.push(attr_name.split(".")[0]);
      });
    });
    return array_utils.uniq(roles);
  }

  _subscribeComponentToChildrenChanges() {
    var roles = this._extractChildComponentRoles();
    ...
    // that in the future.
    roles.forEach(r => {
      var child = this.component.findFirstChildByRole(r);
      if(!child.publish_changes_for.includes("change"))
        this.component.findFirstChildByRole(r).publish_changes_for.push("change");
      this.component.event_handlers.add({ event: "change", role: r, handler: () => this.applyChanges() });
    }, this);
  }

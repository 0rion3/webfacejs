...
export class StateManager {
  ...
  constructor({ component=null, alias_manager=null, states=[], settings={}}={}) {
    ...
    this.children_roles_and_attrs = this._extractChildComponentRolesAndAttrs(this.states);
  }
  ...
  applyTransition() {
    return new PublicPromise(); // Resolves or rejects when transition is either completed or discarded.
                                // This method should be reloaded the descendant class and make use of that promise.
  }
  ...
  _pickTransitionsFromMatches(matches) { // old name pickEntitiesForMatches
    ...
    return { in: [...new Set(resulting_transitions.in)], out: [...new Set(resulting_transitions.out)] };
  }
  ...
  _extractChildComponentRolesAndAttrs(states) {
    var roles_and_attrs = {};
    states.forEach(declaration => {
  ...
  _subscribeComponentToChildrenChanges() {
    ...
    Object.keys(this.children_roles_and_attrs).forEach(role => {
      ...
      this.component.event_handlers.add({ event: "change", role: role, handler: () => this.applyTransition() });
    }, this);
  }

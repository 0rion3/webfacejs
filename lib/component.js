...
import { arraysIntersect     } from './utils/array_utils.js'
...
import { DisplayStateManager } from './services/state/display_state_manager.js'
...
export class Component extends extend_as("Component").mixins(
  ...
  static createFromTemplate({ name=null, template_name=null, container=null, attrs={} }={}) {
  ...
  constructor(options=null) {
    ...
    this.attribute_callbacks = {
      'default' : (attr_name, self) => {
        ...
        if(self.display_state_manager)
          self.display_state_manager.applyChanges();
      }
    };
  ...
  afterInitialize() {
    ...
    if(this.display_states) this.display_state_manager = new DisplayStateManager(this);
  ...
  updateAttributes(names_and_values, { callback=true, ignore_non_existent=false }={}) {
    if(this.display_state_manager != null) this.display_state_manager.lock = true;
    ...
    super.updateAttributes(names_and_values, { callback: callback, ignore_non_existent: ignore_non_existent });
    ...
    if(this.publish_changes_for && (this.publish_changes_for == "#all" ||
      arraysIntersect(this.publish_changes_for, Object.keys(names_and_values))
    ))
      this.publishEvent("change");

    if(this.display_state_manager != null) {
      this.display_state_manager.lock = false;
      if(callback) this.display_state_manager.applyChanges();
    }
  }

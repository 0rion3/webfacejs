...
export class EditableSelectComponent extends extend_as("EditableSelectComponent").mix(SelectComponent).with() {
  ...
  constructor() {
    ...
    this.attribute_callbacks["input_value"] = (attr_name, self) => {
      ...
      self.set("display_value", self.options.get(self.get("input_value")));
      if(self.get("display_value") == null)
        self.set("display_value", self.get("input_value"));
  ...
  afterInitialize() {
    ...
    this.updatePropertiesFromNodes({ attrs: ["allow_custom_value", "query_param_name"], invoke_callbacks: false });
  ...
  /** Filters options by value user typed into the field.
    ...
    */
  filterOptions() {
    this.options = LinkedHashMap.from(this.original_options.map);

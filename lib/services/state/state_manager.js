...
export default class StateManager {
  ...
  //
  _pickTransitionsFromMatches(matches) {
  ...
  _attrHasAcceptableValue(attr_name, v) {
    ...
    if(typeof v === "string" && !v.endsWith("()") || v.constructor === Array) {
      ...
      v.constructor.name === "Map") {
        for(let assertion_name in v) {
          let assertion_values = prepareValues(v[assertion_name]);
          if(!assert[assertion_name](c.get(attr_name), assertion_values)) return false;
        }
        return true;
      } else {
        return v === c.get(attr_name);

...
export class StateManager {
  ...
  constructor({ component=null, alias_manager=null, states=[], settings={}}={}) {
    ...
    this.current_transitions = { in: [], out: [], previous_out: []};
  }
  ...
  pickTransitionsForState() {
    ...
    // Update transitions that are supposed to be applied (if they changed).
    // This is so we don't run the same transitions when the state hasn't actually changed, or it changed,
    // but the transitions for the same state are the same as for the old one.
    if(
      !array_utils.equals(this.current_transitions.in, transitions.in) ||
      !array_utils.equals(this.current_transitions.out, transitions.out)
    ) {
      var previous_out = this.current_transitions.out;
      this.current_transitions = transitions;
      this.current_transitions.previous_out = previous_out;
    }
  ...
  applyTransitions({ transitions=this.pickTransitionsForState(), external_promise=null}={}) {
    ...
    // Not going to call transitions again if they're the same as before.
    if(transitions.called) return new Promise(resolve => resolve);
    ...
    if(external_promise)
      external_promise.then(() => this.applyTransitionsInSequence(
        [transitions.previous_out, transitions.in], transition_promise
      ));
    else
  ...
  _pickTransitionsFromMatches(matches) { // old name pickEntitiesForMatches
    ...
    for(let k in resulting_transitions) {
      resulting_transitions[k] = array_utils.uniq(resulting_transitions[k], { remove_null: true });
    }

...
export default class StateManager {
  ...
  _pickTransitionsFromMatches(matches) { // old name pickEntitiesForMatches
    ...
    return this._transitionsToApplyAfterStateChange({
  ...
  _attrHasAcceptableValue(attr_name, v) {
    ...
    if(typeof v === "string" && !v.endsWith("()") || v.constructor === Array) {
      let values = string_utils.toArrayOrObject(v);
      ...
      v.constructor.name === "Map") {
        for(let k in v)
          if(!assert[k](c.get(attr_name), string_utils.toArrayOrObject(v[k]))) return false;
        return true;
      } else {
        return v === c.get(attr_name);
      }

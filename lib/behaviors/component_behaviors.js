...
export class ComponentBehaviors extends extend_as("ComponentBehaviors").mixins() {
  ...
  showPart(part_name, animation_speed, { display_value = "block" } = {}) {
    return this._animationOnPart("show", part_name, animation_speed, { display_value: display_value });
  }
  async blinkPart(part_name, animation_speed, { display_value = "block" } = {}) {
    await this.hidePart(part_name, 0);
    return this.showPart(part_name, animation_speed, { display_value: display_value });
  }

  _animationOnPart(animation_name, part_name, animation_speed, { display_value = "block" } = {}) {
    var promises = [];
    this.component.findAllParts(part_name).forEach(part => {
      promises.push(this.animator[animation_name](part, animation_speed, { display_value }));
    }, this);

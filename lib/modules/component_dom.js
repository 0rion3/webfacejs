...
const ComponentDom = (ComponentDom) => class extends ComponentDom {
  ...
  updateAttrsFromNodes({ attrs=false, run_callback=false }={}) {
    ...
    attrs.forEach(function(a) {
      this._readAttrFromNode(a, { run_callback: run_callback });
    }, this);
  }
  ...
  updatePropertiesFromNodes({ attrs=false, run_callback=false }={}) {
    ...
    return this.updateAttrsFromNodes({ attrs: attrs, run_callback: run_callback });
  }
  // ******* end of support for DEPRECATED code ********************
  ...
  _readAttrFromNode(attr_name, { run_callback=false }={}) {
    ...
    if(attr_el != null) {
      ...
      if(v != null) this.set(attr_name, this._castAttrFromDom(attr_name, v), { run_callback: run_callback });
    }
  }
  ...
  */
  ...
  _findChildComponentDomElements(node) {
    ...
    Array.from(node.children).forEach(function(c) {
      if(c.getAttribute('data-component-class') == null && c.getAttribute("data-component-template") == null)

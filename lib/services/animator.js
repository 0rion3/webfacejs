...
export const Animator = {
  ...
  show: function(elements, ms, { display_value="block" }={}) {
    return this._applyToCollection(elements, (el) => {
      return this.animateEl(
        ...
        ms, { core_animation: "show", display_value: display_value }
      );
    });
  },
  ...
  hide: function(elements, ms) {
    return this._applyToCollection(elements, (el) => {
      const validMs = ms === 0 ? 1 : ms
      return this.animateEl(
        el, [{ opacity: 1 }, { opacity : 0 }],
        validMs, { core_animation: "hide" }
      );
    });
  },
  ...
  animateEl: function(el, properties, ms, { core_animation="show", display_value="block" }={}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if(window.webface.document_hidden) {
        ...
        var animation = el.animate(properties, ms);
        animation.onfinish = () => {

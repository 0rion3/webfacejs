...
export class SelectComponent extends extend_as("SelectComponent").mix(Component).with() {
  ...
  constructor() {
    ...
    this.default_attribute_values = { hide_null_option_after_option_selected: true };
    this.native_events = [
  ...
  afterInitialize() {
    ...
    this.updateAttrsFromNodes({ attrs: ["disabled", "input_value"], invoke_callbacks: true });
    ...
    this.attribute_callbacks["input_value"] = (attr_name, self) => self.setDisplayValueFromCurrentInputValue();
  ...
  setDisplayValueFromCurrentInputValue() {
    let ip = this.get("input_value");
    this.set("display_value", this.options.get(ip));

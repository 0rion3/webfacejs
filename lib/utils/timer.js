  ...
  humanTimeAsString({
    ...
    units_reversed.forEach((u,i) => {
      if(ht[u] != null) {
        ...
        // If the suffix is ":", which is standard hours:minutes:seconds separator and it's also
        // the smalles unit, we don't add the suffix to the end (which is something you'd expect).
        let suffix = (i == units_reversed.indexOf(smallest_unit) && unit_suffixes[u] == ":") ? "" : unit_suffixes[u];

...
export class DisplayStateManager extends StateManager {
  ...
  applyTransitionNow(transitions, transition_promise) {
    ...
    var display_transition_promise = new PublicPromise();
    display_transition_promise
      .then(() => transition_promise.resolve())
      .catch(() => transition_promise.resolve());

    this.queue.push({
      display_state: [
        this.entities.filter(e => !transitions.includes(e)),
        transitions
      ], transition_promise: display_transition_promise
    });

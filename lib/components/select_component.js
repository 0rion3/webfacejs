...
export class SelectComponent extends extend_as("SelectComponent").mix(Component).with() {
  ...
  constructor() {
    ...
    this.attribute_names  = ["display_value", "input_value", "disabled", "name", "fetch_url", "separators_below", "top_values", "sort_on_fetch", "spinner_el"];
    this.native_events    = [`selectbox.${this.click_event.join(",")}`, "keypress", "keydown", "option.click"];
    ...
    this.attribute_callbacks = {
      'default' : (attr_name, self) => Component.attribute_callbacks_collection['write_attr_to_dom'](attr_name, self)
    };
  ...
  afterInitialize() {
    ...
    this.updateAttrsFromNodes({ attrs: ["display_value", "name", "fetch_url", "separators_below", "top_values", "sort_on_fetch", "spinner_el"],invoke_callbacks: false });
    this.updateAttrsFromNodes({ attrs: ["disabled"], invoke_callbacks: true });

    if(isBlank(this.get("display_value"))) {
      this.updateAttrsFromNodes({ attrs: ["input_value"], invoke_callbacks: true });
  ...
  setValueByInputValue(ip) {
    ...
    this.set("display_value", (ip == null ? "" : this.options.get(ip)));   this.set("input_value", ip);
    this.focused_option = ip; // I'm not entirely sure why I repeated this line here. Perhaps some callback changes that.
    // Do not publish "change" event unless input_value actually changed.

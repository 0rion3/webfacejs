...
export class DisplayStateManager {
  ...
  _expandFoldedStates(display_states, parent=null) {
    ...
    display_states.forEach(ds => {
      ...
      ds[1] = string_to_array_or_object(ds[1]);
      if(parent) {
        ds[0] = { ...parent[0], ...ds[0] };
        ds[1] = string_to_array_or_object(parent[1]).concat(ds[1]);
        ds[1] = array_utils.uniq(ds[1]);
      }

      expanded_display_states.push(ds);
      if(ds[2]) { // declaration Array has a third element, means it has folded states
        let folded_states = ds.pop();
        expanded_display_states = expanded_display_states.concat(this._expandFoldedStates(folded_states, ds));
      }

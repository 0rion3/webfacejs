...
export class Component extends extend_as("Component").mixins(
  ...
  constructor(options=null) {
    ...
    this.attribute_callbacks = {
      'default' : (attr_name, self) => {
        ...
        if(attr_name == "payment_window_expired")
          console.log("Updating payment_window_expired to " + self.get(attr_name));
  ...
  afterInitialize() {
    ...
    if(this.states) this.state_dispatcher = new StateDispatcher(this);
  ...
  updateAttributes(names_and_values, { callback=true, ignore_non_existent=false }={}) {
    ...
    if(this.state_dispatcher != null) {
      ...
      if(callback) this.state_dispatcher.applyTransitions();
    }
  }

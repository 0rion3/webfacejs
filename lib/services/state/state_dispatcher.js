...
import { arrayMove           } from "../../utils/array_utils.js";
...
export class StateDispatcher {
  ...
  applyTransitions() {
    ...
    this._sortStateManagersByBeforeAfter(state_managers_and_picked_transitions).forEach((i) => {
      let sm          = i[0];
      let transitions = i[1].in;
      promises.push(this.state_managers.get(sm).applyTransitions({
        transitions: transitions,
        external_promise: promises[promises.length-1]
      }));
    });

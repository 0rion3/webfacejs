...
export class DisplayStateManager {
  ...
  _extractChildComponentRolesAndAttrs() {
    var roles_and_attrs = {};
    this.display_states.forEach(declaration => {
      Object.keys(declaration[0]).forEach((attr_name) => {
        if(attr_name.includes(".")) {
          let role_and_attr = attr_name.split(".");
          let role = role_and_attr[0];
          let attr = role_and_attr[1];
          if(roles_and_attrs[role] == null)
            roles_and_attrs[role] = [];
          if(!roles_and_attrs[role].includes(attr))
            roles_and_attrs[role].push(attr);
        }
      });
    });
    return roles_and_attrs;
  }

  _subscribeComponentToChildrenChanges() {
    var roles_and_attrs = this._extractChildComponentRolesAndAttrs();
   ...
   */
  _applyLatestChanges(display_state) {
    if(!display_state) return;

    this.queue.push(display_state);
    // If the queue is longer than 1, then some other promise is already
    // awaiting the completion of the previous promise. We simply pushed
    // the latest state into the queue and that other promise will take care of it
    // because it always uses the last state in the queue and disregards all others.
    // We don't need to create another promise here.
    if(this.queue.length > 1) return;

    if(this.last_promise == null)
      this.last_promise = new Promise(resolve => resolve());

    var self = this;
    this.last_promise.then(() => {
      if(self.queue.length == 0) return;
      var display_state = self.queue.pop();
      self.queue = [];
      this.last_promise = Promise.all([
        self.applyAction(self.settings.default_state_clear_action, display_state[0]),
        self.applyAction(self.settings.default_state_action, display_state[1])
      ]);
    });
  ...
  _expandFoldedStates(display_states, parent=null) {
    ...
    display_states.forEach(ds => {
      ...
      this._expandConditionSet(ds).forEach((ds_from_set) => {
        var condition_set = ds_from_set[0];
        if(typeof condition_set === "string")
          condition_set = this.component.component_states[condition_set];
        let folded_states = ds_from_set[2];
        ds_from_set = [condition_set, string_to_array_or_object(ds_from_set[1])];

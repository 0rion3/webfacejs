...
export class Timer {
  ...
  resume() {
    if(this.paused_at != null) {
      if(this.finish_at)
        this.finish_at = this.finish_at + (Timer.time_now_in_ms - this.paused_at);
      this.paused_at = null;
...
export class CountdownTimer extends Timer {
  ...
  get current_time() {
    var t = this._roundTimeToStep(this.finish_at - Timer.time_now_in_ms);
    if(t < 0) return 0;
    else      return t; 
  }
...
export class StopwatchTimer extends Timer {
  ...
  get current_time() {
    var t = this._roundTimeToStep(Timer.time_now_in_ms - this.start_at);
    if(t > this.finish_at)
      return finish_at;
    else
      return t;
